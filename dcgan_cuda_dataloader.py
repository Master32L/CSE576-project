...
def train():    
    ...
    for epoch in range(epochs): # We iterate over epochs.
    ...
    #        netG = netG.cuda()
            ...
            print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f' % (epoch, epochs, i, len(dataloader), errD.data[0], errG.data[0])) # We print les losses of the discriminator (Loss_D) and the generator (Loss_G).
